...
Status: v0.9.0 implementation slice
...
4. 그 결과를 원본 실행, Finding, artifact, SHA-256, Trace ID까지 다시 열 수
   있는가.
5. 측정값·사용자 추정·AI 추론·확인 불가를 섞지 않았는가.
...
| 검증된 효과 | 측정 또는 회귀 방지 + 원본 연결 + artifact + 채택 commit + 재검증 run/commit | 재현 가능한 개선으로 세는 최소 조건 |
...
| 기록된 시간 절감 | `metricKey=time_saved`인 측정 또는 사용자 추정 효과 중 같은 단위의 합 | 운영자가 기록한 시간 변화. 다른 지표나 혼합 단위는 섞지 않음 |
...
Assurance Effect
        ↓
baseline → observed value → adoption commit → reverification run
```
...
참조가 없으면 기록은 남기지만 검증된 효과에는 포함하지 않습니다.
...
완결되려면 원본 근거, 사람의 채택, 채택된 commit에서의 재검증을 별도로
기록해야 합니다. 현재 효과 기록 API는 이 메타데이터를 받을 수 있지만, AI가
patch를 채택하거나 commit/push하는 동작은 수행하지 않습니다.
