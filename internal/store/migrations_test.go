...
func TestMigrationTwelveAddsActionRunsFromVersionEleven(t *testing.T) {
	db := openUnmigratedTestDatabase(t, "action-runs-v11-forward")
	defer db.Close()
	...
	var count int
	if err := db.QueryRow(`SELECT count(*) FROM sqlite_master WHERE type='table' AND name='action_runs'`).Scan(&count); err != nil || count != 1 {
		t.Fatalf("action runs migration = %d, %v", count, err)
	}
}
